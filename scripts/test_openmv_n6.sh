#!/usr/bin/env bash
# test_openmv_n6.sh — Phase 3 hardware smoke test (Spec §4 Phase 3 exit, §24, §25).
#
# Run ON the Pi from the repo root:  ./scripts/test_openmv_n6.sh [SERIAL]
# Deploys the capture service to the N6, then exercises the Phase 3 exit criteria:
...
"$PY" -m host_tools.deploy_openmv --board n6 --serial "$SERIAL"
...
N6_SERIAL="$SERIAL" "$PY" -m pytest tests/hardware/test_openmv_n6.py -v -p no:cacheprovider

echo "PASS: N6 capture service validated on hardware."
